...
def elabora_campionato(chiave, meta, stagione):
    ...
    # Il filtro per data ha senso solo sulla stagione in corso.
    # Su una stagione passata restituirebbe zero partite, perché la finestra
    # di ricerca cadrebbe fuori dal periodo in cui si è effettivamente giocato.
    if stagione == stagione_corrente():
        oggi = datetime.now(timezone.utc).date()
        params = {
            "from": (oggi - timedelta(days=14)).isoformat(),
            "to": (oggi + timedelta(days=14)).isoformat(),
        }
        print("  Finestra: ultimi/prossimi 14 giorni")
    else:
        params = {}
        print("  Stagione passata: scarico l'intero calendario")

    partite_raw = api_get("fixtures", league=meta["id"], season=stagione, **params)
    ...
    if not partite_raw:
        print("  Zero partite restituite dall'API per questa stagione")
        return {**meta, "stagione": stagione, "concluse": [], "inCorso": [],
                "prossime": [], "errore": "nessuna partita disponibile"}
