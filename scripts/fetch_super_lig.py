"""
Archivio Süper Lig — statistiche disciplinari e designazioni arbitrali.

Due fonti:
  1. football-data.co.uk (D1.csv) — cartellini e falli di squadra.
     Licenza PDDL. Il campo arbitro esiste ma è vuoto.
  2. TFF — designazioni ufficiali della federazione turca, con arbitro,
     assistenti e quarto uomo. Il VAR non compare in questa pagina.

Le condizioni d'uso del sito TFF consentono di pubblicare le informazioni
citando la fonte, purché non a scopo commerciale. L'attribuzione compare
nella pagina, e le richieste sono due a settimana.
...
import atamalar as dg

RADICE = Path(__file__).resolve().parent.parent
ARCHIVIO = RADICE / "data" / "super_lig.csv"
DESIGNAZIONI = RADICE / "data" / "atamalar_super_lig.json"
USCITA = RADICE / "docs" / "data_super_lig.json"

BASE_CSV = "https://www.football-data.co.uk/mmz4281"
CODICE = "T1"
CATEGORIA = "Süper Lig"
PROGRAMMA = "https://www.tff.org/default.aspx?pageID=600"
UA = "Mozilla/5.0 (compatible; PureStats/1.0)"

# football-data cambia nel tempo la denominazione di alcune squadre.
ALIAS_CSV = {
    "buyuksehyr": "Buyuksehyr", "basaksehir": "Buyuksehyr",
    "istanbul basaksehir": "Buyuksehyr",
    "goztep": "Goztep", "goztepe": "Goztep",
    "corum": "Corum", "corum fk": "Corum",
    "amedspor": "Amedspor", "amed sk": "Amedspor",
    "ad. demirspor": "Ad. Demirspor", "adana demirspor": "Ad. Demirspor",
    "karagumruk": "Karagumruk", "fatih karagumruk": "Karagumruk",
    "buyuksehir": "Buyuksehyr", "eintracht frankfurt": "Ein Frankfurt", "koln": "FC Koln", "cologne": "FC Koln", "mgladbach": "M'gladbach", "hamburger sv": "Hamburg", "bayer leverkusen": "Leverkusen", "schalke": "Schalke 04", "st. pauli": "St Pauli", "hertha bsc": "Hertha", "nuremberg": "Nurnberg", "fortuna dusseldorf": "Dusseldorf",
}
...
def scarica(url, descrizione, binario=False):
    ...
    # Il sito TFF usa windows-1254, la codifica turca: senza di essa
    # i nomi arriverebbero illeggibili.
    for codifica in ("utf-8-sig", "utf-8", "windows-1254", "latin-1"):
        try:
...
def aggiorna_designazioni(anno, designazioni_note):
    ...
    html = scarica(PROGRAMMA, "designazioni TFF")
    if html:
    ...
    manuale = RADICE / "data" / "atamalar.txt"
    if manuale.exists():
        try:
...
def calcola_statistiche(righe, future):
    ...
    return {
        ...
        "fonte": "football-data.co.uk (PDDL) · designazioni TFF",
